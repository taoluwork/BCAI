...
.then(()=>{
    ...
    fs.open('./cpu.txt', 'w', function(err){
        if(err) throw err;
    })
    ...
    process.on('SIGINT', async () => {
        try{
            ...
            if(response.val.toLowerCase() == "quit")
            {
                ...
                {
                    fs.appendFileSync('./cpu.txt', "\nCPU usage is "+String(monitor.getCpuUsage())+"%, memory usage is "+String(monitor.getMemTotal())+" MB and the CLI ran for "+String(Math.round(stopwatch.read()/1000))+" s\n", function(err){
                        if(err) throw err;
                    })
                    console.log("\nCPU usage is "+monitor.getCpuUsage()+"%, memory usage is "+monitor.getMemTotal()+" MB, and the CLI ran for "+String(Math.round(stopwatch.read()/1000))+" s\n")
                    stopwatch.stop();
                    monitor.stop();
                    process.exit(-1);
                }
